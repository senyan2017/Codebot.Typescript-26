/// <reference path="../../typings/boot.resource.d.ts" />
...
function executeScripts(element: HTMLElement) {
    ...
    function nodeNameEquals(elem: Node, name: string): boolean {
        return elem.nodeName != null && elem.nodeName.toUpperCase() == name.toUpperCase();
    }

    function evalScript(elem: HTMLScriptElement): void {
    ...
    for (var i = 0; children[i]; i++) {
        ...
        if (nodeNameEquals(child, "script") && (!child.getAttribute("type") || child.getAttribute("type")!.toLowerCase() == "text/javascript"))
            scripts.push(child as HTMLScriptElement);
    }
...
function loadScript(url: string, callback: Proc): void {
    resourceLoader.loadScript(url, callback);
}

/** A dictionary of CSS property names to their values. */
interface StyleMap {
 ...
 */
function setStyle(query: QuerySelect, styles: StyleMap): void {
    ...
    for (let e of elements) {
        ...
        for (let k of keys) {
            let value = styles[k];
            style[k] = isNumber(value) ? value + "px" : value as string;
        }
    }
}
 ...
 */
function addEvent(query: QuerySelect | EventCapable, name: string, event: EventListenerOrEventListenerObject) {
...
/** Load a stylesheet asynchronously, with deduplication via ResourceLoader.
 * @param href The URL of the stylesheet to load.
 * @param onload Optional callback invoked when the stylesheet has loaded.
 */
function addStyleSheet(href: string, onload?: Proc): void {
    resourceLoader.loadStyleSheet(href, onload);
}

/** Load a javascript file asynchronously, with deduplication via ResourceLoader.
 * @param src The URL of the script to load.
 * @param onload Optional callback invoked when the script has loaded.
 */
function addJavaScript(src: string, onload?: Proc): void {
    resourceLoader.loadScript(src, onload);
}
